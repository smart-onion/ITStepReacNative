...
export default function SignUpView({setPageMode}:{setPageMode:React.Dispatch<React.SetStateAction<string>>}) {   // Register
    ...
    useEffect(() => {
        ...
        const numberIsValid = (Number(userFormData.phone.slice(1)) % 1) == 0 && 
                                userFormData.phone.startsWith("+") && 
                                userFormData.phone.length == 13
        setFormValid(
        ...
        if (userFormData.name.length < 2){
            validationErrors.push("Name lenght is less then 3 characters")
        }
        ...
        if (!numberIsValid){
            validationErrors.push("Invalid number")
        }

        if(userFormData.password.length < 3){
            validationErrors.push("Password lenght is less then 3 characters")
        } 
        if(userFormData.password != userFormData.repeatPassword){
            validationErrors.push("Passwords not match")
        }
        validationErrors = validationErrors.filter((v, i) => validationErrors.indexOf(v) == i)
    ...
    return <>
        ...
        <View style={AuthStyle.authRow}>
            <Text style={AuthStyle.authRowText}>Repeat Пароль</Text>
            <TextInput secureTextEntry={true} style={AuthStyle.authRowInput} 
                value={userFormData.repeatPassword} 
                onChangeText={t => setUserFormData({...userFormData, repeatPassword:t})} />
        </View>
