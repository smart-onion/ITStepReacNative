...
export default function Anim() {               // з ініціалізацією початковим значенням (тут - 1)
    ...
    // home work
    const myOpacity     = useRef(new Animated.Value(0.6)).current;
    const myTransX      = useRef(new Animated.Value(0)).current;
    const myTransY      = useRef(new Animated.Value(0)).current;
    const myRotate      = useRef(new Animated.Value(0)).current;
    const myScale       = useRef(new Animated.Value(1)).current;
    const myRotationDeg = myRotate.interpolate({
        inputRange: [0, 1],
        outputRange: ['0deg', '30deg'],  // Map 0 to 0deg, 1 to 30deg
    });
    const myAnimationPress = () => {
        Animated.parallel([
            // for moving
            Animated.sequence([
                
                Animated.timing(myTransX,{
                    toValue: 10,
                    useNativeDriver: true,
                    duration: 300,
                }),
                Animated.timing(myTransY,{
                    toValue: -10,
                    useNativeDriver: true,
                    duration: 300
                }),
    ...
    return <View style={AnimStyle.animLayout}>
        ...
        <View style={AnimStyle.animRow}>
            ...
            <View style={AnimStyle.animItem}>
                <Pressable onPress={myAnimationPress}>
                    <Animated.View style={{
                        opacity: myOpacity,
                        transform:[
                            {translateX: myTransX},
                ...
                <Text style={AnimStyle.animLabel}>Home Work</Text>
